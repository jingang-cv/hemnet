...
class WhitenLoss(nn.Module):
    ...
    def forward(self, f_map_real, mask_matrix_real, num_remove_cov_real,  margin_real=0, eye_real=None):
        '''
         Input:
...
def get_covariance_matrix(f_map, eye=None):
    '''
     ...
     '''
    eps = 1e-5
    # f_map = f_map.permute(0, 2, 1)  # 'b n d -> b d n'
    B, C, N = f_map.shape  # i-th feature size (B X C X H X W)
    if eye is None:
        eye = torch.eye(C).cuda()
    f_map = f_map.contiguous().view(B, C, -1)  # B X C X H X W > B X C X (H X W)
    f_cor = torch.bmm(f_map, f_map.transpose(1, 2)).div(N-1) + (eps * eye)  # C X C / HW
